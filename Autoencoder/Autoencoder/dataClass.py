...
class Data(object):
	...
	def loadLabels(self):
		...
		testName = ["nvidia-07","rose","tekken","eizenman"]
		validationName = ["fitsum","nvidia-08"]
		test = list()
		for file in files:
			...
			if (name + ".jpg") in self.Dictdata:
				if 'PupilCenter' in self.Dictdata[name + ".jpg"]:
	...
	def getBatchData(self,batch):
		...
		for image in (batch):
			...
			'''
			mask = np.expand_dims(image_reader, axis=2)
			self.labelData.append((mask).astype(np.float)/255.0)
